...
CREATE TABLE task (
    ...
    task_id VARCHAR(50) NOT NULL, /* 任务编号（格式：类型缩写-年份-序号，如RP-2024-001） */
    ...
    price_confirmed TINYINT DEFAULT 0, /* 客户是否已确认报价（0-未确认，1-已确认） */
    need_site_visit TINYINT DEFAULT NULL, /* 是否需要上门（0-不需要，1-需要） */
    visit_appointment_time DATETIME2, /* 约定上门时间 */
...
BEGIN
    ...
    INSERT INTO task (id, task_id, title, task_type, customer_id, customer, customer_level, location, contact_person, contact_phone, sales_id, priority, status, 
                 start_date, deadline, device_name, device_model, device_brand, description,  current_step) VALUES
